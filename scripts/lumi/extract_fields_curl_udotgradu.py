...
def extract_fields_with_curl(
    ...
    with h5py.File(hdf5_file, "r") as f:
        ...
        if include_udotgradu_mag:
            ...
            sinT = np.sin(theta)[None, :, None]           # (1,ntheta,1)
            cosT = np.cos(theta)[None, :, None]
            cotT = cosT / np.maximum(sinT, 1e-14)         # avoid singular at poles

            # Derivatives wrt r and theta (allow non-uniform grids)
            du_r_dr,     du_r_dtheta     = np.gradient(u_r,     r, theta, axis=(0, 1), edge_order=2)
            du_theta_dr, du_theta_dtheta = np.gradient(u_theta, r, theta, axis=(0, 1), edge_order=2)
            du_phi_dr,   du_phi_dtheta   = np.gradient(u_phi,   r, theta, axis=(0, 1), edge_order=2)

            # Derivatives wrt phi (assume uniform phi grid; enforce periodic)
            if nphi < 3:
                raise ValueError("Need nphi >= 3 to compute periodic d/dphi reliably.")
            dphi = float(phi[1] - phi[0])
            # sanity: (optional) you can check uniformity if you want
            du_r_dphi     = _d_dphi_periodic(u_r,     dphi, axis=2)
            du_theta_dphi = _d_dphi_periodic(u_theta, dphi, axis=2)
            du_phi_dphi   = _d_dphi_periodic(u_phi,   dphi, axis=2)

            # Convective acceleration components in spherical coordinates:
            # a = (u dot grad)u
            # theta here is colatitude.
            inv_r = 1.0 / np.maximum(rr, 1e-14)
            inv_r_sin = 1.0 / np.maximum(rr * sinT, 1e-14)

            adv_r = (
                u_r * du_r_dr
                + (u_theta * inv_r) * du_r_dtheta
                + (u_phi * inv_r_sin) * du_r_dphi
                - (u_theta**2 + u_phi**2) * inv_r
            )

            adv_theta = (
                u_r * du_theta_dr
                + (u_theta * inv_r) * du_theta_dtheta
                + (u_phi * inv_r_sin) * du_theta_dphi
                + (u_r * u_theta) * inv_r
                - (u_phi**2) * cotT * inv_r
            )
            ...
            adv_phi = (
                u_r * du_phi_dr
                + (u_theta * inv_r) * du_phi_dtheta
                + (u_phi * inv_r_sin) * du_phi_dphi
                + (u_r * u_phi) * inv_r
                + (u_theta * u_phi) * cotT * inv_r
            )
