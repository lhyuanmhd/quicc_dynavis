...
def plot_snapshot_panel(case_dir: Path, data, save_path: Path,
    ...
    try:
        ...
        Ek, Ra = ts.input_params_from_path(str(case_dir))
    except Exception:
        Ek, Ra = _input_params_from_path(case_dir)

    fig = plt.figure(figsize=(24, 10))
    gs = fig.add_gridspec(2, 4, width_ratios=[1, 1, 1, 0.6], wspace=0.1, hspace=0.25)
   ...
   #ax13 = fig.add_subplot(gs[1, 3])
    ...
    fields_snapshot.plot_equatorial(str(case_dir), data, "u_theta", ax=ax02)
    fields_snapshot.plot_meridional(str(case_dir), data, "u_phi", atphi=atphi, ax=ax03)
   
    fields_snapshot.plot_equatorial(str(case_dir), data, "T", title="T_0 + T", ax=ax10, include_background=True)
    fields_snapshot.plot_equatorial(str(case_dir), data, "T", title="T", ax=ax11, include_background=False)
    fields_snapshot.plot_equatorial(str(case_dir), data, "curl_u_axial", ax=ax12)
    ...
    time = data["time"]
    fig.suptitle("Ek={}, Ra={}, time={:.5e}".format(Ek, Ra, float(time)),
                 y=0.98, fontsize=16)
...
def _plot_one_visu_dir(visu_dir: Path,
    ...
    if out is None:
        ...
        try:
            ...
            Ek, Ra = _input_params_from_path(case_dir)
        #out = fig_dir / "Ek_{}_q{}_Ra{}_{}_{}_snapshots.pdf".format(Ek, q, Ra, run_dir.name, tag)
        if udotgradu:
            out = fig_dir / "Ek_{}_Ra{}_{}_{}_udotgradu.pdf".format(Ek, Ra, run_dir.name, tag)
        else:
            out = fig_dir / "Ek_{}_Ra{}_{}_{}_snapshots.pdf".format(Ek, Ra, run_dir.name, tag)
    else:
        out = out.resolve()
...
def main():
    ...
    for t in chosen_tags:
        ...
        out = fig_dir / "Ek_{}_Ra{}_{}_{}_snapshots.pdf".format(Ek, Ra, run_dir.name, t)
